...
class blazeFaceDetector():
	...
	def initializeModel(self, type):
		if type == "front":
			model_file = os.path.join(current_path, "./weights/face_detection_front.tflite")
		elif type =="back":
			model_file = os.path.join(current_path, "./weights/face_detection_back.tflite")
		else:
			raise ValueError(f"Unsupported model type: {type}")

		# On Windows, tflite may fail to open non-ASCII paths.
		# Load model bytes directly to avoid path encoding issues.
